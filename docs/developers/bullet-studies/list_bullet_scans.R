...
list_scans_by_bullet <- function(study, bullet_dir) {
  df <- data.frame(
    study = study,
    filename = list.files(bullet_dir, recursive = TRUE, full.names = TRUE, pattern = "\\.x3p")
  )
  return(df)
}
...
# Houston studies drop GEA folders
csv_dir <- "docs/developers/bullet-studies/data"
file <- file.path(csv_dir, "Houston Set Final scans.csv")
df <- read.csv(file)
